...
% Start a parallel pool using the local cores you requested on the HPC
poolobj = gcp('nocreate'); % Check if pool exists
if isempty(poolobj)
    parpool('local'); % This will automatically use the cores you requested
end
...
num_vars = length(lb);
moniker = "pop3000_gen75_singleaxis_newCROPNEW";
...
if num_targets > 1
  ...
  title('Energy Generation'); ylabel('kWh/year'); grid on;
    ...
    saveas(fig, "single_objective_comparisons" + moniker + ".png");
...
saveas(fig_layout, "optimal_layout_comparison" + moniker + ".png");
...
if agriParams.tracking_mode == 1
    ...
    saveas(fig_track, "optimal_tracking_curves" + moniker + ".png");
