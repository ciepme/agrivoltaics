classdef spider_plot_class < matlab.graphics.chartcontainer.ChartContainer & ...
    ...
    properties(Access = public, SetObservable)
        ...
        AxesOffset (1, 1) double {mustBeGreaterThan(AxesOffset, 0), mustBeLessThan(AxesOffset, 1)} = 0.3 % Axes offset
    ...
    methods (Access = protected)
        ...
        function initialize(obj)
            ...
            %%% Polar Axes %%%
            % Polar coordinates
            rho_increment = 1/obj.AxesInterval;
            rho = 0:rho_increment:1;
            rho = rho * (1 - rho_offset) + rho_offset;
            ...
            if strcmp(obj.AxesRadial, 'on')
                ...
                for ii = 1:length(theta)-1
                    ...
                    [x_axes, y_axes] = pol2cart(theta(ii), [0, 1]);
            ...
            if strcmp(obj.AxesWeb, 'on')
                ...
                for ii = 1:length(rho)
                    ...
                    % Plot
                    obj.RhoAxesLines(ii) = line(x_axes, y_axes,...
                        'Parent', ax,...
            ...
            if strcmp(obj.MinorGrid, 'on')
                % Polar coordinates
                rho_minor_increment = 1/(obj.AxesInterval*obj.MinorGridInterval);
            ...
            if strcmp(obj.AxesShaded, 'on')
                ...
                for ii = 1:num_shaded
                    ...
                    for jj = 1:2:length(x_points)-2
                        ...
                        % Patch polygon
                        obj.AxesPatches(jj) = patch(...
                            'Parent', ax,...
                            'Faces', f, 'Vertices', v,...
                            'FaceColor', obj.AxesShadedColor{ii},...
                            'EdgeColor', 'none',...
                            'FaceAlpha', obj.AxesShadedTransparency(ii));
                        obj.AxesPatches(jj).Annotation.LegendInformation.IconDisplayStyle = 'off';
                    end
                end
            end
            ...
            for ii = 1:theta_end_index
                ...
                for jj = 1:length(rho)
                    ...
                    if axes_direction_index(ii)
                        % Axes increment value
                        axes_value = min_value - (range/obj.AxesInterval) * (jj-1);
                    else
                        % Axes increment value
                        axes_value = min_value + (range/obj.AxesInterval) * (jj-1);
                    end
