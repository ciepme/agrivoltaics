...
load('x_star.mat');                  % loads x_star and best_fval
f_star = -best_fval;                 % positive social value at optimum
expected_crop_elec_price = 0.054;
agriParams.crop_elec_price = expected_crop_elec_price;
fprintf('f_star = $%.2f\n', f_star);
fprintf('Using sensitivity baseline crop_elec_price = %.3f $/kWh\n', ...
    agriParams.crop_elec_price);
...
if use_parallel
    ...
    % having each of the parallel workers run the same setup commands
    pctRunOnAll("agrivoltaics_variable_definition; load_system('agrivoltaics_v1'); set_param('agrivoltaics_v1','InitFcn',''); set_param('agrivoltaics_v1','SimulationMode','normal');");
end
...
base_dv_names = {'z_p','l_p','w_p','phi','sigma','y_p','x_p'};
n_dv = numel(x_star);
dv_names = cell(1, n_dv);
for i = 1:min(7, n_dv)
    dv_names{i} = base_dv_names{i};
end
if n_dv > 7
    for i = 8:n_dv
        dv_names{i} = sprintf('tracking_%02d', i - 7);
    end
end
...
all_x_new_dv  = zeros(0, n_dv);  % n_evals x n_dv
all_var_idx   = zeros(0, 1);     % which DV was perturbed
